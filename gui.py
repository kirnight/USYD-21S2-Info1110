...
class GUI:
    ...
    def update_frame(self, spaceship, asteroid_ls, bullet_ls, score, fuel):
        ...
        spaceship_pos = offset(spaceship.get_xy(), -config.radius["spaceship"])
        self.screen.blit(spaceship_image, spaceship_pos)

        # Display Asteroid
        for asteroid in asteroid_ls:
            if asteroid.obj_type == "asteroid_small":
                ast_pos = offset(asteroid.get_xy(), -config.radius["asteroid_small"])
            if asteroid.obj_type == "asteroid_large":
                ast_pos = offset(asteroid.get_xy(), -config.radius["asteroid_large"])
        ...
        for bullet in bullet_ls:
            pygame.draw.circle(self.screen, gold, bullet.get_xy(), config.radius["bullet"])
